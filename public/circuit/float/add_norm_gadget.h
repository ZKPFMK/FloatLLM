...
public:
  ...
  add_norm_gadget(libsnark::protoboard<Fr>& pb,
      ...
      shift2.reset(new shift_gadget(pb, man2, -offset2->ret()+M+3, misc::Log2UB(M+3)));
      ...
      shift.reset(new shift_gadget(pb, man->ret(), offset, misc::Log2UB(M+2)));
  ...
  libsnark::linear_combination_array<Fr> ret() const { return shift_bits->pack1->bits; }
  ...
  std::shared_ptr<shift_gadget> shift;
  std::shared_ptr<min_gadget> offset2;
  std::shared_ptr<shift_gadget> shift2;
